...
// beta variance priors
  real<lower=0> sigma_beta_1_age;
  real<lower=0> sigma_beta_t_1_age;
  real<lower=0> sigma_beta_s_1_age;   
...
model {
  ...
  // priors on variability in beta
  sigma_beta_1_age ~ normal(0,1);
  sigma_beta_t_1_age ~ normal(0,1);
  sigma_beta_s_1_age ~ normal(0,1);
...
//  sigma_beta_t_continuous ~ normal(0,1);
  ...
  for (i in 1:2){ //11){ 
    ...
    beta[3] ~ normal(0,sigma_beta_1_age);//cauchy(0,10);
    beta_t[3] ~ normal(0,sigma_beta_t_1_age);//cauchy(0,10);
    beta_s[3] ~ normal(0,sigma_beta_s_1_age);//cauchy(0,10); 

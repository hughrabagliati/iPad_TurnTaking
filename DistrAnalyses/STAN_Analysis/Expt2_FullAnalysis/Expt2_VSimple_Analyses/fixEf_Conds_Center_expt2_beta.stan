...
beta0 ~ normal(0,.5); //double_exponential
beta_t0 ~ normal(0,.5);
beta_s0 ~ normal(0,.5);//cauchy(150,10); 

// priors on remaining betas. got worse fit when i estimated a separate variance parameter for each beta.
for (i in 1:4){ //11){ 
beta[i] ~ normal(0,.5);//cauchy(0,10);
beta_t[i] ~ normal(0,.5);//cauchy(0,10);
beta_s[i] ~ normal(0,.5);//cauchy(0,10); 
}

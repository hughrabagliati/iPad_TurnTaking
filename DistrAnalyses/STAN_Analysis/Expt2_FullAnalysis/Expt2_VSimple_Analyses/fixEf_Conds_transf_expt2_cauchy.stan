...
//  sigma_beta_t_continuous ~ normal(0,1);
  ...
  beta0 ~ cauchy(0,sigma_beta0_1); //double_exponential
  beta_t0 ~ cauchy(0,sigma_beta0_t_1);
  beta_s0 ~ cauchy(0,sigma_beta0_s_1);//cauchy(150,10); 
 
  // priors on remaining betas. got worse fit when i estimated a separate variance parameter for each beta.
  for (i in 1:3){ //11){ 
    beta[i] ~ cauchy(0,sigma_beta_1);//cauchy(0,10);
    beta_t[i] ~ cauchy(0,sigma_beta_t_1);//cauchy(0,10);
    beta_s[i] ~ cauchy(0,sigma_beta_s_1);//cauchy(0,10); 
  }

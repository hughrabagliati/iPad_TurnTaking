...
//  real<lower = 0> tau;
//  tau <- inv(lambda);
//  real<lower = 0> sigma_dev[N];
//  for (i in 1:N){
//    sigma_dev[i] <- sigma_e[i] + u_s[Subj[i]];
//    }
  vector[N] mu;
  vector[N] lambda;
  vector[N] tau;
  ...
  lambda[i] <- beta_t[1] + beta_t[2] * factor1[i]+ beta_t[3] * factor2[i]+ beta_t[4] * factor3[i] + beta_t[5] * factor4[i] + 
  ...
  sigma_e[i] <- beta_s[1] + beta_s[2] * factor1[i]+ beta_s[3] * factor2[i]+ beta_s[4] * factor3[i] + beta_s[5] * factor4[i] + 
  ...
  tau[i] <- inv(lambda[i]);
  sigma_dev[i] <- sqrt(sigma_e[i] + u_s[Subj[i]]);
  }
}

model {

  beta[1] ~ double_exponential(400,10);
  beta_t[1] ~ double_exponential(0,10);
  beta_s[1] ~ double_exponential(0,10);
  for (k in 2:12){
    beta[k] ~ double_exponential(0,10);
    beta_t[k] ~ double_exponential(0,10);
    beta_s[k] ~ double_exponential(0,10); 
  }
  u ~ cauchy(0,sigma_u);
//  u_t ~ cauchy(0,sigma_u_t);
  u_s ~ normal(0,sigma_u_s);
  
  rt ~ exp_mod_normal(mu,sigma_dev,tau);
}